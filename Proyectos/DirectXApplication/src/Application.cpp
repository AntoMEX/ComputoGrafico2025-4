...
void Application::setupShaders()
{
	...
	ThrowIfFailed(device->CreateGraphicsPipelineState(&pso_desc, IID_PPV_ARGS(&pipeline_state)), "Error creating pipeline)");
...
void Application::update()
{
	...
	//sceneConstants.projection = XMMatrixTranspose(DirectX::XMMatrixPerspectiveFovLH(DirectX::XMConvertToRadians(60.0f), aspect, 0.1f, 1000.0f));
	sceneConstants.projection = DirectX::XMMatrixIdentity();
	DirectX::XMVECTOR rotationAxis = DirectX::XMVectorSet(0.0f, 0.0f, 1.0f, 1.0f);
	sceneConstants.model = DirectX::XMMatrixRotationAxis(rotationAxis, DirectX::XMConvertToRadians(sceneConstants.triangleAngle)); //Rotación
	//sceneConstants.model = XMMatrixTranspose(DirectX::XMMatrixScaling(0.5f,0.5f,0.5f)); //Cambiar tamaño
}
...
void Application::draw()
{
	...
	D3D12_CPU_DESCRIPTOR_HANDLE rtvHandle = rtvHeap->GetCPUDescriptorHandleForHeapStart(); //Apunta a la posición 0 o al final donde termina el primer buffer
	rtvHandle.ptr += back_buffer_index * rtvIncrementSize; 

	// Clear the render target
	float clearColor[] = { 0.0f, 0.2f, 0.4f, 1.0f };
	commandList->ClearRenderTargetView(rtvHandle, clearColor, 0, nullptr); //Borrar el buffer trasero

	// Set viewport and scissor
	D3D12_VIEWPORT viewport = { 0.0f, 0.0f, static_cast<float>(WINDOW_WIDTH), static_cast<float>(WINDOW_HEIGHT), 0.0f, 1.0f }; //Tamaño
	D3D12_RECT scissor_rect = { 0, 0, WINDOW_WIDTH, WINDOW_HEIGHT }; //No dibujar fuera del espacio de la ventana
	commandList->RSSetViewports(1, &viewport); //Set de viewport
	commandList->RSSetScissorRects(1, &scissor_rect); 
	commandList->OMSetRenderTargets(1, &rtvHandle, FALSE, nullptr); //Dirección de inicio
	commandList->IASetPrimitiveTopology(D3D_PRIMITIVE_TOPOLOGY_TRIANGLELIST); //Topologia de triangulo
	commandList->SetGraphicsRootSignature(rootSignature.Get()); 
	commandList->SetPipelineState(pipelineState.Get()); //Seleccion de shaders
	/*
	//Triangulo
	//Pass parameters
	commandList->SetGraphicsRoot32BitConstant(0, triangle_angle, 0); //agrega un bloque de memoria de 32 bits

	// Draw the triangle
	commandList->DrawInstanced(3, 1, 0, 0); //Dibujo
	*/
	/*
	//Otro triangulo feliz
	//int triangle_angle2 = 0;
	////Pass parameters
	//commandList->SetGraphicsRoot32BitConstant(0, triangle_angle2, 0); //agrega un bloque de memoria de 32 bits
	...
	*/
	...
	//Copiar los datos de la estructura 
	...
	{
		D3D12_RESOURCE_BARRIER barrier = {}; //El que antes estaba detras ahora se activa en la pantalla
		barrier.Type = D3D12_RESOURCE_BARRIER_TYPE_TRANSITION;
		barrier.Flags = D3D12_RESOURCE_BARRIER_FLAG_NONE;
		barrier.Transition.pResource = renderTargets[back_buffer_index].Get();
