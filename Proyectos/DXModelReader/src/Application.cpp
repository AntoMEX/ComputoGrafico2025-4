...
void Application::moveCallback(GLFWwindow* window, double xpos, double ypos)
{
    int width, height;
    glfwGetWindowSize(window, &width, &height);

    //Posición del mouse en un rango definido [-1, 1] usando el tamaño de la ventana
    float normX = (float(xpos) / width) * 2.0f - 1.0f; //-1 izquierda y +1 derecha
    float normY = (float(ypos) / height) * 2.0f - 1.0f; //-1 arriba y +1 abajo

    yaw = -normX * 180.0f;
    pitch = -normY * 180.0f;
}

void Application::scaleCallback(GLFWwindow* window, double xoffset, double yoffset)
{
...
void Application::update() {
    ...
    XMMATRIX scaleMatrix = XMMatrixScaling(zoom, zoom, zoom);

    XMMATRIX rotY = XMMatrixRotationY(XMConvertToRadians(yaw));
    XMMATRIX rotX = XMMatrixRotationX(XMConvertToRadians(pitch));
    
    sceneConstants.model = rotY * rotX * scaleMatrix/*XMMatrixRotationAxis(axis, angleRad) * XMConvertToRadians(sceneConstants.triangleAngle)*/;

    memcpy(mappedMemory, &sceneConstants, sizeof(SceneConstants));// Sizeof es la cantidad de bits a copiar

    //XMMATRIX modelMatrix = rotY * rotX * scaleMatrix;
}
